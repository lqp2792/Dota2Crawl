...
//			System.out.println("Table: " + list.size());
			list = list.extractAllNodesThatMatch(new HasAttributeFilter("width", "100%"));
//			System.out.println("Table: " + list.size());
			
			for(int i=0, n=list.size(); i<n; i++) {
				TableTag table = (TableTag) list.elementAt(i);
//				System.out.println("\tTable " + (i+1) + ": Child: " + table.getChildCount());
				Parser p = new Parser(table.getChildren().toHtml());
				NodeList childList = p.extractAllNodesThatMatch(new NodeClassFilter(LinkTag.class));
				childList = childList.extractAllNodesThatMatch(new NotFilter(new LinkStringFilter("version")));
...
//					System.out.println("\t\t\tHeroesLink: " + heroesLink.size());
					if(heroesLink.size() == 0) {
						...
						if(!isExisted) heroesLink.put(id.getAndIncrement(), link);
					}		
				}
			}
