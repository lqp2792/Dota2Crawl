...
public class SimpleVisitor extends NodeVisitor {
	...
	public SimpleVisitor(HeroStat heroStat, Hero hero, List<LevelStat> levelStats) {
	...
	public void visitTag(Tag tag) {
		if (tag instanceof TableRow) {
			...
			for(TableColumn tc : tcs) {
				...
				if(prevText != null) {
					...
					case "Hit Points":
						...
						level++;
						System.out.println("Set Hit points at level: " + levelStat.getLevel() + 
								" is " + levelStat.getHitPoints());
						break;
					case "Mana":
					...
					case "Damage":
						tokens = new StringTokenizer(presentText, "‒");
						if(tokens.hasMoreTokens()) {
							levelStat.setStartDamage(Integer.parseInt(tokens.nextToken()));
							levelStat.setEndDamage(Integer.parseInt(tokens.nextToken()));
						}
						level++;
						System.out.println("Set Damage at level: " + levelStat.getLevel() + 
								" is " + levelStat.getStartDamage() + "-" + levelStat.getEndDamage());
						break;
					case "Armor":
						levelStat.setArmor(Double.parseDouble(presentText));
						level++;
						System.out.println("Set Armor at level: " + levelStat.getLevel() + 
								" is " + levelStat.getArmor());
						break;
					case "Attacks / Second":
						levelStat.setAttackSpeed(Double.parseDouble(presentText));
						level++;
						System.out.println("Set Attacks / Second at level: " + levelStat.getLevel() + 
								" is " + levelStat.getAttackSpeed());
						break;
					}
				}
				if(level == 0) {
			...
			StringTokenizer tokens = null;
			try {
				Parser parser = ParserUtils.createParserParsingAnInputString(th.getStringText());
				...
				if(imgNodes.size() != 0) {
					...
					case "Strength":
						...
						System.out.println("Set Beginning Strength: " + heroStat.getBeginStrength());
						System.out.println("Set Strength per level: " + heroStat.getStrPerLevel());
						break;
					case "Agility":
						heroStat.setBeginAgility(Double.parseDouble(tokens.nextToken()));
						heroStat.setAgiPerLevel(Double.parseDouble(tokens.nextToken()));
						System.out.println("Set Beginning Agility: " + heroStat.getBeginAgility());
						System.out.println("Set Agility per level: " + heroStat.getAgiPerLevel());
						break;
					case "Intelligence":
						heroStat.setBeginIntel(Double.parseDouble(tokens.nextToken()));
						heroStat.setIntPerLevel(Double.parseDouble(tokens.nextToken()));
						System.out.println("Set Beginning Intelligence: " + heroStat.getBeginIntel());
						System.out.println("Set Intelligence per level: " + heroStat.getIntPerLevel());
						break;
					}
//					System.out.println(it.getImageURL());
				} else {
					parser = ParserUtils.createParserParsingAnInputString(th.getStringText());
